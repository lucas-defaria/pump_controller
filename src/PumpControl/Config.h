...
namespace Config {
    ...
    // an external PWM signal for this long before falling back to MAP control.
    // Useful when the external PWM source takes longer to come online than the
    // Arduino. Each pulseIn attempt blocks up to ~100ms, so this window typically
    // allows 4-5 detection attempts at 25Hz.
